...
- **手动批量抓取-动态数据**：在本页直接多选 `outputs/video_pool/full_site_floorings/` 下的一个或多个现成视频列表 CSV，并可选叠加一个或多个 `uid_expansion` 任务目录中的 `videolist_part_*.csv`；系统会将这些来源合并后按「追踪窗口」筛选、去重，再通过**后台子进程任务**抓取评论/互动量等实时数据并上传至 BigQuery。
- **手动批量抓取-媒体/元数据**：支持两种补抓模式，并且现在将“媒体数据”和“元数据”彻底拆分为独立任务。模式 A 会分别维护待补媒体清单与待补元数据清单；模式 B 则分别支持“按上传清单抓取媒体数据”和“按上传清单抓取元数据”。其中媒体任务支持设置 `音视频截断长度（秒）`，默认只抓每个视频开头 `30` 秒，设为 `0` 时则完整抓取。四类任务均支持**后台子进程执行**与**随时停止**。
...
- `bilibili-datahub.py` 当前主要包含十个主标签页：
  ...
  - **手动批量抓取-媒体/元数据**：分别维护待补媒体数据 / 待补元数据清单，或基于手动上传清单去重后分别补抓媒体文件与视频元数据。
  - **BigQuery / GCS 数据查看**：查看结构化数据和媒体资产，并导出媒体文件。
  - **快捷跳转**：打开视频页或作者主页。
  - **tid 与分区名称对应**：查询分区映射。

- Google Cloud 配置和自动批量抓取配置会分别保存在本地 `.local/` 下的 DataHub 配置文件中；B 站 Cookie 现在也可由用户显式保存到本地 `.local/bilibili-datahub.cookie.txt`，供后台任务读取与热更新。
...
5. 到 `手动批量抓取-媒体/元数据` 页先执行一次 `同步待补媒体数据视频清单` 或 `同步待补元数据视频清单`，再视需要触发模式 A 或模式 B，确认对应任务目录和清单文件已经生成。
...
- `手动批量抓取-媒体/元数据` 看不到待补条目：先确认当前侧边栏填写的是目标 BigQuery Dataset；模式 A 的待补元数据清单只会纳入已进入 `video_stat_snapshots` 或 `topn_comment_snapshots`、但尚未写入 `videos` 的视频；待补媒体清单则只会纳入尚未具备 `assets(video+audio)` 的视频。
- `手动批量抓取-媒体/元数据` 想中途停止：可直接在页面中的“当前后台任务状态”区域点击停止按钮；任务会在安全检查点尽快停止，并把状态写入对应任务目录。
...
- **每轮流程（概要）**：读取所选 `full_site_floorings` 视频列表 CSV → 读取所选 `uid_expansion` 任务目录中的 `videolist_part_*.csv` → 合并候选视频 → 按本轮启动时刻与「追踪窗口」过滤发布时间 → 去重 → 启动后台任务并生成本轮抓取清单 → 后台批次抓取实时数据并入库。
- **窗口裁剪说明**：无有效 `pubdate` 的条目不会进入或留在当前有效 watchlist 中。
- **后台执行说明**：页面重跑或关闭浏览器页签后，只要 `streamlit` 进程仍在，后台任务会继续执行；页面重新打开后可在页内读取最近登记的后台任务状态。
- **cookie 热更新说明**：后台任务不会在每个 `bvid` 前都重读 cookie，而是按批次读取 `.local/bilibili-datahub.cookie.txt`；每批开始前刷新一次 `Credential`，并在批内出现疑似登录态/风控导致的剩余 CSV 时，直接用最新 cookie 继续处理后续批次。
- **每轮会话目录** `manual_crawl_stat_comment_<时间戳>/` 中会保存 `filtered_video_list.csv`、`manual_crawl_state.json` 与 `logs/`；若底层批量任务部分失败，同目录下还会保留剩余 CSV 供继续执行。后台任务层另外会在 `outputs/video_data/background_tasks/` 下保存任务配置与状态文件。
- **风控睡眠说明**：发现阶段若识别为风控错误，会按当前设置持续睡眠后重试，直到成功或被用户手动中断；非风控错误则直接中止本轮任务。

### 手动批量抓取-媒体/元数据说明

- 本页专门处理**一次性数据**，即视频元数据与媒体文件（视频轨 + 音频轨），且现在已经拆分成互不耦合的两类任务。
- **模式 A：基于数据集缺失条目**
  - 点击 `同步待补元数据视频清单` 后，程序会在当前 BigQuery Dataset 中计算：候选集仍然是已进入 `video_stat_snapshots` 或 `topn_comment_snapshots` 的视频，但只把**尚未写入 `videos` 表**的 `bvid` 纳入待补元数据清单。
  - 点击 `同步待补媒体数据视频清单` 后，程序会计算同一批候选视频中**尚未同时具备 `assets(video+audio)`** 的 `bvid`，并写入待补媒体清单。
  - 元数据任务与媒体任务会分别启动独立后台任务，分别落到各自的 `manual_crawl_meta_mode_A_<date>_<time>/` 与 `manual_crawl_media_mode_A_<date>_<time>/` 目录中。
- **模式 B：基于手动上传清单**
  - 页面上方模块负责“按上传清单抓取媒体数据”，下方模块负责“按上传清单抓取元数据”。
  - 两个模块都支持上传一个或多个包含 `bvid` 列的 CSV/XLSX 文件；系统会先拼接去重，再按当前 Dataset 中对应目标的完成情况剔除已完成条目。
  - 媒体模块会额外暴露 `音视频截断长度（秒）` 参数：设为正整数时，仅保存每个视频开头 `N` 秒内的音频与视频；设为 `0` 时完整抓取。
- **执行与停止逻辑**
  - 媒体任务仅抓取媒体文件，不再顺带抓元数据；元数据任务也只抓 `videos` 侧结构化信息，不再顺带抓媒体。
  - 页面中的停止按钮会向当前后台任务写入停止请求；runner 会在批次切换、视频切换和媒体分块读取阶段检查该请求，并尽快以 `stopped` 状态结束。
  - 命中风控时不再走“sleep 后续跑”的旧逻辑，而是直接结束当前 part / 任务并保留剩余清单，便于后续继续测试或重新触发。
- 四类任务都会保留任务级状态文件、批量抓取状态文件和日志文件，便于中断后排查与续跑；后台任务层另外会保留最近登记的任务状态，供前端刷新查看。
