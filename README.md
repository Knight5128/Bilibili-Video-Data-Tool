...
当前 `Bilibili DataHub` 的本地后台任务会周期性写入心跳状态；当遇到 BigQuery 短暂的 SSL / 连接抖动时，也会对流式写入做额外重试。Windows 下启动后台 worker 时也会额外申请脱离父进程 job 约束，尽量减少前端刷新或宿主终端波动把后台任务一并带停的情况。手动点击停止后，后台任务状态会先从 `running/queued` 切换为 `stopping`，待安全退出后再写成 `stopped`，这样 `task_status.json` 能更准确反映任务当前阶段。

当前 `手动批量抓取-媒体/元数据` 页面已经拆分为独立的媒体数据任务与元数据任务：模式 A 分别维护“待补媒体数据清单”和“待补元数据清单”，模式 B 也分别提供“按上传清单抓取媒体数据”和“按上传清单抓取元数据”两套入口。其中媒体任务支持设置 `音视频截断长度（秒）`，默认 `30` 秒，设为 `0` 时表示完整抓取；四类后台任务都支持在页面中随时发送停止请求。
