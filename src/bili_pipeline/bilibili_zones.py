...
_ROW_RE = re.compile(r"^\|\s*(?P<name>[^|]+?)\s*\|\s*(?P<code>[^|]+?)\s*\|\s*(?P<tid>\d+)\s*\|.*?(?P<route>\[[^\]]+\]\([^)]+\)|/[^|]+)?\s*\|\s*$")
_ROUTE_MD_RE = re.compile(r"\[([^\]]+)\]\(([^)]+)\)")


def _parse_route(cell: str | None) -> str | None:
    if not cell:
        return None
    cell = cell.strip()
    m = _ROUTE_MD_RE.search(cell)
    if m:
...
def parse_zones_md(md: str) -> list[ZoneEntry]:
    main: str | None = None
    out: list[ZoneEntry] = []
    for raw in md.splitlines():
        ...
        if m_sec:
            main = m_sec.group(1).strip()
            continue

        if line.startswith("|") and main and not line.startswith("| ---"):
            ...
            out.append(
                ZoneEntry(
                    ...
                    code=m_row.group("code").strip(),
                    tid=int(m_row.group("tid")),
                    route=_parse_route(m_row.group("route")),
                )
            )
    return out
...
@lru_cache(maxsize=1)
def list_zones() -> tuple[ZoneEntry, ...]:
    return tuple(parse_zones_md(_ZONES_MD))


def find_by_tid(tid: int) -> tuple[ZoneEntry, ...]:
    return tuple(z for z in list_zones() if z.tid == tid)


def unique_mains(zones: Iterable[ZoneEntry] | None = None) -> list[str]:
    zs = zones if zones is not None else list_zones()
    mains = {z.main for z in zs}
    return sorted(mains)
